...
/*   Updated: 2026/09/23 17:57:15 by kmoriyam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int ac, char *av[])
{
	...
	set_icmp_header(&tr);
	ssize_t sent;
	sent = sendto(tr.sock_fd, &tr.icmp, sizeof(tr.icmp), 0, \
		(struct sockaddr *)&tr.sock_in, sizeof(tr.sock_in));
	if (sent == -1)
	{
		perror("sendto");
		freeaddrinfo(tr.ai);
	...
	from_len = sizeof(tr.sock_in);
	received = recvfrom(tr.sock_fd, buf, sizeof(tr.icmp), 0, \
