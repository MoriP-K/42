...
2. Edit `.env` and configure the following variables:
    ...
    - `VITE_API_URL` – Backend API URL (default: `https://oekakinomori.com` when using HTTPS)
    - For Google OAuth: `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REDIRECT_URI`, `FRONTEND_URL` (`https://oekakinomori.com`)

### Running the Project (HTTPS)

The backend is served over HTTPS via Nginx as required by the 42 curriculum.

1. For local testing, add `oekakinomori.com` to your hosts file:

    ```bash
    # /etc/hosts
    127.0.0.1 oekakinomori.com
    ```

2. Make `start.sh` executable (first time only):

    ```bash
    chmod +x scripts/start.sh
    ```

3. Start all services (certificates are generated automatically if missing):

    ```bash
    ./scripts/start.sh
    ```

    **Note**: Do not run `docker compose up -d` directly without certificates. NGINX will fail to start because it cannot find the SSL certificate files. Use `./scripts/start.sh` instead, which generates certificates before starting.

    Or manually: `./scripts/generate-certs.sh` then `docker compose up -d`

4. Access the application:
    - **Application**: https://oekakinomori.com (port 443)
    - **Database**: localhost:5432 (PostgreSQL)

5. **Browser certificate warning**: Self-signed certificates will show a "Your connection is not private" warning. Click "Advanced" → "Proceed to oekakinomori.com (unsafe)" to continue.

6. **Google OAuth**: Register these redirect URIs in Google Cloud Console:
    - `https://oekakinomori.com/v1/auth/google/callback`

7. **Certificate expiration**: The self-signed certificate generated by `generate-certs.sh` is valid for 1 year. If it expires, delete `nginx/certs/cert.pem` and `nginx/certs/key.pem`, then run `./scripts/start.sh` again to regenerate.
